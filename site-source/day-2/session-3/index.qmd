...
hex_canvas <- ggplot(mapping = aes(x, y)) + 
  ...
  scale_fill_gradientn(colours = palette2, guide = guide_none())
...
base + 
  ...
  as_reference("polygons", id = "shift") + 
  with_displacement(
    text,
    x_map = ch_alpha("shift"),
    y_map = ch_alpha("shift"), 
    x_scale = 150,
    y_scale = -150
  )
```
...
poly <- polygon_layer(c(-4, 2, 2), c(0, 0, 6), alpha = 1)
...
blend_canvas +
  as_group(tree_layer, leaf_layer, id = "flametree") + 
  with_blend(poly, bg_layer = "flametree", blend_type = "xor")
```
